...
function init() {
    ...
    animateObjectRotation(object); // Inicia a animação de rotação do objeto
        ...
        setTimeout(() => {
            hideLoadingScreen();
        }, 400); // Mantém a tela de carregamento por 3 segundos
    }, undefined, (error) => {
        console.error('Erro ao carregar o modelo', error);
        hideLoadingScreen();
    });

    // Função para animar a rotação do objeto
function animateObjectRotation(object) {
    // A cada quadro, rotaciona o objeto no eixo Y
    function rotate() {
        object.rotation.y += 0.01; // Velocidade da rotação (ajuste conforme necessário)
        requestAnimationFrame(rotate); // Chama novamente a função no próximo quadro
    }
    rotate(); // Começa a animação de rotação
}
...
function hideLoadingScreen() {
    ...
    // Desabilita os controles de órbita
    controls.enableRotate = false; // Impede a rotação do modelo
    controls.enableZoom = false;  // Impede o zoom
}
